...
async def process_txt_to_vcf(files_data: List[bytes], batch_size: int, user_id: int) -> Tuple[List[str], Dict]:
    ...
    for i in range(0, len(unique_numbers), batch_size):
        ...
        for number in batch:
            uid = str(uuid.uuid4())
            name = f"Contact {number.replace(' ', '')[-8:]}"
        ...
        filename = f"contacts_batch_{i//batch_size + 1}_{int(time.time())}.vcf"
...
async def handle_batch_size(message: Message, state: FSMContext):
    ...
    try:
        ...
        files_count = len(user_cache[user_id]["files"])
        await message.answer("📊 **Menganalisis file...**")
        
        result_files, info = await process_txt_to_vcf(user_cache[user_id]["files"], batch_size, user_id)
